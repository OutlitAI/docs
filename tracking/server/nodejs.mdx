...
<CodeGroup>
```bash npm
npm install @outlit/tracker-node
```

```bash pnpm
pnpm add @outlit/tracker-node
```

```bash yarn
yarn add @outlit/tracker-node
```
</CodeGroup>
...
export async function POST(request: Request) {
  const { userId, email, newPlan } = await request.json()

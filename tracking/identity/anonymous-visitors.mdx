...
While anonymous, these events are stored in `VisitorEvent`:
...
Form submissions are captured with sanitization:

```typescript
// Form submitted with email = "jane@acme.com", name = "Jane"
// Stored in VisitorEvent with formData
{
  "type": "form",
  "formId": "contact-form",
  "formFields": {
    ...
    "name": "Jane",
    "company": "Acme Inc"
    // password, credit_card, etc. are automatically removed
  }
}
```
...
1. **Find or create CustomerContact** with the provided email
2. **Link AnonymousVisitor** by setting `resolvedToContactId`
3. **Background job processes** historical events:
   - Converts VisitorEvents to CustomerActivity
   - Preserves original timestamps
   - Adds to unified timeline

...
description: "Track customers from first visit through product engagement, with separate billing status tracking"
---
...
```mermaid
%%{init: {'theme': 'neutral', 'themeVariables': { 'primaryColor': '#6366f1', 'primaryTextColor': '#fff', 'primaryBorderColor': '#4f46e5', 'lineColor': '#818cf8' }}}%%
flowchart TB
    subgraph contact["Contact Journey (per person)"]
    ...
    subgraph billing["Account Billing (per company)"]
        F[None] --> G[Trialing]
        G --> H[Paying]
        H -.-> I[Churned]
        I -.->|re-subscribes| H
...
| **Activated** | Completed onboarding or key milestone | Call `activate()` |
| **Engaged** | Actively using the product | Automatic or `engaged()` |
...
You don't need to call `engaged()` manually in most cases. However, you can call it explicitly for specific milestones:

```typescript
outlit.engaged({ milestone: 'completed_first_project' })
```
...
async function handleOnboardingComplete() {
  ...
  outlit.activate({ flow: 'onboarding_v2' })
}
```

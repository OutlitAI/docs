...
For server-rendered apps (Next.js, Remix), pass the user identity from your auth system as a prop. This ensures the user is identified immediately without a client-side round-trip.

```tsx
// app/layout.tsx (Next.js App Router with Auth)
import { OutlitProvider } from '@outlit/tracker/react'
...
<Info>
  When the `user` prop changes (login/logout), the provider automatically calls `setUser()` or `clearUser()` to update the identity.
</Info>
...
<ParamField path="activate" type="(properties?: object) => void">
  Mark the current user as activated.
</ParamField>

<ParamField path="engaged" type="(properties?: object) => void">
  Mark the current user as engaged.
</ParamField>

<ParamField path="paid" type="(properties?: object) => void">
  Mark the current user as paid.
</ParamField>

<ParamField path="churned" type="(properties?: object) => void">
